#!/usr/bin/env bash
set -euo pipefail

# Upload a workbook, poll the job, and save the two documents in two formats:
#   $RUN_DIR/json/context.json  graph.json  trace.json
#   $RUN_DIR/md/context.md      graph.md    trace.md
# Usage: run-context-job.sh [path/to/model.xlsx]
# Env: BASE_URL, OUT_DIR, RUN_DIR, JOB_TIMEOUT_SEC

source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/common.sh"
need_cmds
ensure_run_dir
mkdir -p "$RUN_DIR/json" "$RUN_DIR/md"

workbook="$(resolve_workbook "${1:-}")"
log_summary "workbook=${workbook}"

code="$(http_post_file /v1/context-jobs "$RUN_DIR/post-job.json" "$workbook")"
if [[ "$code" != "200" && "$code" != "202" ]]; then
  echo "POST /v1/context-jobs expected 200 or 202, got ${code}" >&2
  exit 1
fi

job_id="$(json_field "$RUN_DIR/post-job.json" job_id)"
if [[ -z "$job_id" ]]; then
  echo "POST response missing job_id" >&2
  exit 1
fi
log_summary "job_id=${job_id}"

job_status="$(poll_job "$job_id" "$RUN_DIR/job-status.json")"
log_summary "job_status=${job_status}"

if [[ "$job_status" == "failed" ]]; then
  echo "job ${job_id} failed; status saved to ${RUN_DIR}/job-status.json" >&2
  exit 1
fi

context_url="$(json_field "$RUN_DIR/job-status.json" context_json_url)"
if [[ "$context_url" != "/v1/context-jobs/${job_id}/context.json" ]]; then
  echo "job status missing context_json_url for ${job_id}" >&2
  exit 1
fi
context_md_url="$(json_field "$RUN_DIR/job-status.json" context_md_url)"
if [[ "$context_md_url" != "/v1/context-jobs/${job_id}/context.md" ]]; then
  echo "job status missing context_md_url for ${job_id}" >&2
  exit 1
fi
graph_url="$(json_field "$RUN_DIR/job-status.json" graph_json_url)"
if [[ "$graph_url" != "/v1/context-jobs/${job_id}/graph.json" ]]; then
  echo "job status missing graph_json_url for ${job_id}" >&2
  exit 1
fi
graph_md_url="$(json_field "$RUN_DIR/job-status.json" graph_md_url)"
if [[ "$graph_md_url" != "/v1/context-jobs/${job_id}/graph.md" ]]; then
  echo "job status missing graph_md_url for ${job_id}" >&2
  exit 1
fi

require_get "/v1/context-jobs/${job_id}/context.json" "$RUN_DIR/json/context.json"
require_get "/v1/context-jobs/${job_id}/context.md" "$RUN_DIR/md/context.md"
require_get "/v1/context-jobs/${job_id}/graph.json" "$RUN_DIR/json/graph.json"
require_get "/v1/context-jobs/${job_id}/graph.md" "$RUN_DIR/md/graph.md"

origin="$(
  python3 "$_COMMON_DIR/check-graph.py" \
    "$RUN_DIR/json/context.json" \
    "$RUN_DIR/json/graph.json" \
    --context-md "$RUN_DIR/md/context.md" \
    --graph-md "$RUN_DIR/md/graph.md" \
    --print-origin
)"
if [[ -n "$origin" ]]; then
  encoded="$(urlencode "$origin")"
  require_get \
    "/v1/context-jobs/${job_id}/graph/trace?from=${encoded}&direction=precedents&depth=6" \
    "$RUN_DIR/json/trace.json"
  require_get \
    "/v1/context-jobs/${job_id}/graph/trace.md?from=${encoded}&direction=precedents&depth=6" \
    "$RUN_DIR/md/trace.md"
  python3 "$_COMMON_DIR/check-graph.py" \
    --trace "$RUN_DIR/json/trace.json" \
    --trace-md "$RUN_DIR/md/trace.md" \
    --origin "$origin"
  log_summary "graph_trace_from=${origin}"
else
  log_summary "graph_trace skipped (no formula cell, concept_id, or row_key)"
fi

echo "OK  job ${job_id} (${job_status})  wrote ${RUN_DIR}"
