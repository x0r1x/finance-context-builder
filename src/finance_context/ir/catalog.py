...
class IrCatalog:
    ...
    def open(cls, audit_dir: Path) -> IrCatalog:
        ...
        graph_edges = audit_dir / "ir" / "graph_edges.parquet"
        if graph_edges.is_file():
            path = _sql_path(graph_edges)
            con.execute(f"CREATE VIEW graph_edges AS SELECT * FROM read_parquet('{path}')")
        return cls(con)
