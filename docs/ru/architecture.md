...
Каждая принятая книга стартует в spawned-процессе, чей главный поток считает весь пайплайн. Дочерний процесс вызывает тот же `configure_logging`, поэтому `stage_done`, `job_reuse` и пропуски моделей видны в stdout контейнера. Повторный POST готовой книги пишет `job_reuse` и процесс не стартует. API пишет `http_start` и `http_done` на каждый запрос, кроме `/healthz`. `finance-context build` остаётся одним процессом. Главный поток ребёнка считает пайплайн. Если эмбеддинги настроены, `TaxonomyPrefetch` считает векторы таксономии параллельно parse, compile и layout. `llm_concurrency` и `embedding_concurrency` больше 1 поднимают пул только внутри этой книги. Общего замка на mapping нет. `MAX_CONCURRENT_JOBS` (по умолчанию 2) ограничивает число детей этого процесса API. Второй POST, пока процесс жив, другой процесс не стартует. Второй POST готовой книги отдаёт снимок, если `publisher` совпадает. Другой `publisher` останавливает старый процесс и стартует новый; пустой штамп его не останавливает. Новый прогон удаляет только устаревший хвост из `meta.stages` (`compile`, затем `layout`, `mapping`, `graph`, `publish`). Без карты стадий хвост прежний: layout, mapping и опубликованные документы, parse и formula IR остаются. `compile` удаляет ещё `raw/` и formula IR. `publish` переписывает context и оставляет mapping и graph. На старте и на GET, если процесса уже нет, книга в `queued` или `running` помечается `failed` с `error` `process_lost` и сама не стартует. Статус берётся из `meta.json` на диске. Поле `queue` у `GET /readyz` равно `in_process`, поле `jobs` — число живых детей. `finance-context serve` запускает uvicorn с `workers=1`: процессы книг — дети этого процесса API. Статусы: `queued`, `running`, `succeeded`, `degraded`, `needs_input`, `failed`.
