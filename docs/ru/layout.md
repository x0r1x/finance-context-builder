...
Одинаковая последовательность ключей зерна `model_year` или `year` делит одну сетку периодов, даже если колонки сдвинуты или шапки лежат на разных листах. Каждая местная ось остаётся в `axes` со своим id и колонками и получает `timeline_id` самой полной оси. В `context.md` это одна таблица: строка на каждую ось (буква колонки под ключом периода) и строки фазы под теми же ключами. Колонка значения остаётся местной: `Y1` на одном листе может быть D, на другом E. Если у обеих осей даты заполнены и расходятся, или два непустых набора флагов различаются, оси остаются разными. `Y1` и `2024` не склеиваются. Разная длина последовательности тоже.
