...
5. **Graph** (`graph.json` schema `1.3.0`, `graph-edges.json`, `graph-dangling.json`, `formulas.json`, `ir/graph_index.parquet`): cell-level formula graph of record. Index joins `period_id` / `row_key` / `concept_id`; `period_lag`, `col_offset`, `dangling_reason`, `status`, `reason`, and `evidence` live on `cell_edges`. `graph.json` is stats, workbook `iterate`, cycle classes (`iterative_ok` vs `unexpected`) with optional `breakers`, `circularity_hints` when SCC is empty, and `dangling_classes` — not the full edge list. Verified blanks (`empty_range_member`, `empty_ref`) are materialized as `node_type=empty` and are not `graph.dangling`. `graph-dangling.json` records `sources` back to the formula cell and A1 range. Trace is `GET /v1/context-jobs/{id}/graph/trace`; JSON edges are `GET .../graph/edges`. Details: [graph.md](graph.md). There is no `report.json`.
