...
2. **Formulas** (`ir/cells.parquet`, `ir/edges.parquet`): templates, AST, dependency edges. Named ranges such as `DS_Drawn_C:DS_Drawn_N` stay unresolved. Unsupported formulas are marked `unparsed`. Binary templates keep operator parentheses, e.g. `(1+Sub_Growth_M)^(R[-4]C[0]-1)`.
3. **Layout** (`layout.json`): statement-like blocks, period axes, grain, row labels, row kinds, and section path.
...
| `index` | `Week #` / `Month #` style counters (small integers across the axis). |
| `helper` | Check / tie-out rows. |

`needs_input` is driven only by questions on `fact` rows.

Weekly dates keep distinct `YYYY-MM-DD` keys. Grain `week` / `biweek` does not collapse them to months.
