...
| `context.md` block | Every row: `Row` (`row_key`), `Path` (`label_path`), Kind, Disposition, Concept (`unknown` on abstain), `Cells` (role cells: `L total: -86400`, `G Start: 01.01.2024`), the formula, and every axis value. Each value shows the cache and, under it, `Sheet!A1`. A period header is `Y23 (AA)` (key and column letter). Under the table, `relations` use the same `row_key`. An empty formula on a row that `graph.md` links to is a defect. Params: `## Parameters / {sheet}`, scenarios as columns `Live Case (L)` / `Case 1 (N)` |
