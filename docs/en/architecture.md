...
Guides: [overview](overview.md), [layout](layout.md), [mapping](mapping.md), [taxonomy](taxonomy.md), [graph](graph.md), [llm](llm.md), [review](review.md).
...
| `index` | Counter `Week #` / `Month #`: a run of `0\|1..n` on the axis and a counter label, or no formulas in the period cells. Kept on the block row. |
...
Each accepted workbook starts a spawned process whose main thread runs the full pipeline. The child calls the same `configure_logging`, so `stage_done`, `job_reuse`, and model skips show up on the container stdout. A repeat POST of a finished book logs `job_reuse` and does not start a process. The API logs `http_start` and `http_done` for every request except `/healthz`. `finance-context build` stays one process. The child's main thread runs the pipeline. When embeddings are configured, `TaxonomyPrefetch` embeds taxonomy labels alongside parse, compile, and layout. `llm_concurrency` and `embedding_concurrency` above 1 start a pool inside that book only. There is no shared mapping lock. `MAX_CONCURRENT_JOBS` (default 2) caps how many children this API process runs. A second POST while that process is alive does not start another. A second POST of a finished workbook returns the snapshot when `publisher` matches. A different `publisher` stops the old process and starts a new one; a missing stamp does not stop it. The new run deletes only the stale tail in `meta.stages` (`compile`, then `layout`, `mapping`, `graph`, `publish`). No stage map keeps the old tail: layout, mapping, and the published documents, while parse and formula IR stay. `compile` also deletes `raw/` and formula IR. `publish` rewrites context and leaves mapping and graph. On startup, and on GET when the process is gone, a job still `queued` or `running` is marked `failed` with `error` `process_lost` and is not started again. Status comes from `meta.json` on disk. `GET /readyz` field `queue` is `in_process`, and `jobs` is the number of live children. `finance-context serve` starts uvicorn with `workers=1`: job processes are children of this API process. Statuses: `queued`, `running`, `succeeded`, `degraded`, `needs_input`, `failed`.

`scripts/run.sh` only polls HTTP. Its `JOB_TIMEOUT_SEC` defaults to 300 when unset and does not stop the child. The server stops the child after Settings `job_timeout_sec` (default 3600, often set in `.env`). `scripts/check-graph.py` checks schema `1.7` for the graph and `1.13` for context.
