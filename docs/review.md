...
| `graph.json` + `graph-edges.json` + `graph-dangling.json` + `GET .../graph/trace` | Cell-level зависимости, `period_lag`, `status=empty` (проверенный blank) отдельно от `status=unresolved`; AST в `formulas.json` / parquet |
