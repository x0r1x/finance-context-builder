...
| `context.json` / `context.md` | Схема `1.12.0`: паспорт, `axes` (единственное место фаз, флагов и `group_key`), каждый блок и каждая строка (`row_key`, `disposition`, `concept_id`, одна формула, `series[].points` по `axis_ids` с `period_key`, `scale_factor`, `period_position`, `aggregation`). Timeline в Markdown — период строкой таблицы, не столбцом. Год, повторённый над месяцами, — `group_key`, не вторая ось. Год и месяц в одной полосе заголовков — одна таблица. Повтор той же шапки в следующей секции ссылается на ту же ось. В Markdown секция `## Axes` печатает ось один раз и прячет пустые колонки: у года без фазы остаётся Period, ровная месячная сетка сжимается до строки на год (`2020 | 2020-01 .. 2020-12`). В JSON у периода нет пустой `phase`, пустого `flags` и `calendar_year`, если год уже в ключе. У таблицы `Axes: ...` и сетка по каждой серии. Рядом с Unit стоит Time (`flow/during_period/sum`), пустая ячейка — `empty`, период вне фазы — `n/a`, тысячи показываются как `k£ ×1000` и `1.5 (1500)`. Заголовок периода — `period_key` и буква колонки (`Y23 (AA)`). Под блоком список `relations` теми же `row_key`. Без `inventory` / `unmapped` / `excluded` и без реестра ячеек. `mapping_stats` (`concept_coverage` и `mapping_quality`), pointer `graph` |
