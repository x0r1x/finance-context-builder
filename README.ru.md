...
Пишет `context.json`, `context.md`, `graph.json` и `graph.md`. Если эти документы уже лежат в каталоге и в `meta.json` тот же хэш `publisher`, команда печатает `reused` и пайплайн не запускает. Нет поля или хэш другой — удаляются layout, mapping и опубликованные документы, затем они собираются заново; parse и formula IR той же книги сохраняются.
...
Отправка книги. `POST` возвращает **202**, и API поднимает процесс для этой книги. Повторный POST, пока процесс жив, второй не стартует, если код публикации на диске всё ещё совпадает с `meta.json`. Повторный POST книги, снимок которой собран этим же кодом, отдаёт этот снимок и не пересобирает. Нет поля `publisher` или хэш другой — живой процесс этой книги останавливается, затем заново собираются layout, mapping и context, parse и formula IR переиспользуются, если `ir/compile.json` совпал, и для строк, которые structure и labels не закрыли, вызываются эмбеддинги/LLM. Опрашивайте, пока `status` не станет терминальным:
